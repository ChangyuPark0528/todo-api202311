...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            // 토큰 위조 검사 및 인증 완료 처리
            if(token != null) {
                // 토큰 서명 위조 검사와 토큰을 파상해서 클레임을 얻어내는 작업
                ...
                authorityList.add(new SimpleGrantedAuthority("role" + userInfo.getRole().toString()));

                // 인증 완료 처리
                // (스프링 시큐리티에게 인증 정보를 전달해서 전역적으로 어플리케이션에서
                // 인증 정보를 활용할 수 있게 설정)
                AbstractAuthenticationToken auth
                        = new UsernamePasswordAuthenticationToken(
                        userInfo, // 컨트롤러에서 활용할 유저 정보
                        null, // 인증된 사용자의 비밀번호 - 보통은 null 값
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        // -- Authorization: Bearer asdsd7dw548d6...
